...
int init_vl53l0x(uint8_t id){
	...
	if(Status == VL53L0X_ERROR_NONE){
		Status =VL53L0X_SetDeviceAddress(&Avoidance_Device[id].Device,Avoidance_Device[i].new_address);
	}
	if(Status == VL53L0X_ERROR_NONE){
		Avoidance_Device[id].Device.I2cDevAddr=Avoidance_Device[i].new_address/2;
		if (ioctl(i2c_fd, I2C_SLAVE,Avoidance_Device[id].Device.I2cDevAddr) < 0){
			printf("Failed to acquire IIc bus access and/or talk to slave.");
			return -1;
		}
